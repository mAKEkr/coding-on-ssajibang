...
국내 Modern PHP User Group(Modern PUG)에서 변역해주신 문서에요. PHP를 배우고싶다면 이 문서로 배우세요.
다른 인터넷에 돌아다니는 최소 2~3년 지난 문서들은 다 함정입니다.
...
- [Javscript(ES6)](https://github.com/lukehoban/es6features)
요즘은 인터넷 브라우저뿐만 아닌 모든곳에서 사용되고있는 Javscript입니다. 그렇지만, 아직 인터넷으로 배우기에는 약간 좀 빡센면이 있는 Javscript에요. 왜냐면
  ...
  3. ES6의 제대로 된 문서조차 거의 없음(ECMAScript6 Sepcification Sheet가 있긴 하지만, 애매모호합니다)
  4. 타 언어들보다 모호한 객체성

때문에, "난 정말로 이걸 배워야해!" 라시는분들이 아니면 추천드리지 않아요. 그렇지만, Node.js라던지 아니면 자바스크립트 코딩에는 정말 좋은 기능들이 많기때문에, 배우시는걸 추천드립니다.
